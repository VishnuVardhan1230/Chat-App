...
const Leftsidebar = () => {
  ...
  // 🔁 Sort by unseen messages first, then alphabetically
 const sortedUsers = [...filteredUsers].sort((a, b) => {
  const userIdA = a?._id?.toString()
  const userIdB = b?._id?.toString()

  const timeA = new Date(latestMessages?.[userIdA]?.timestamp || 0).getTime()
  const timeB = new Date(latestMessages?.[userIdB]?.timestamp || 0).getTime()
